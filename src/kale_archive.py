...
def _process_single_cell_weighted_ranks(
    ...
    tf_summary = grouped.apply(calculate_tf_stats, include_groups=False)  # Don't include the group key in the result)
...
def _process_single_cell_stouffer(
    ...
    priors_group = priors_df.groupby("source", observed=True).agg({"target": list, "weight": list})
    ...
    try:
        ...
        match analysis_method:
            case "ranks_from_zscore":
                ...
                data_for_processing = pd.DataFrame(z_mat, index=adata.obs_names, columns=adata.var_names)

            case "rank_of_ranks":
                print("Using raw gene expression as input for per-cell ranking...")
                data_for_processing = pd.DataFrame(X, index=adata.obs_names, columns=adata.var_names, dtype=float)
                if ignore_zeros:
        ...
        results_df = pd.DataFrame(records, columns=["cell", "regulator", "p_value", "direction"])
        pvalue_df = results_df.pivot(index="cell", columns="regulator", values="p_value")
        activation_df = results_df.pivot(index="cell", columns="regulator", values="direction")
...
def func_kale(
    ...
    Args:
        ...
        weighted (bool, optional): If True, use weighted methods where applicable. Defaults to False.
